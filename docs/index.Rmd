---
title: "Curso de Estadística R"
subtitle: "Estadística simulación en R"
author: 
- name: "Luis Fernando Escobar"
  affiliation: "Centro Integral de Capacitación e Innovación Tecnológica, FICCT-UAGRM"
date: "2024-12-04"
output: 
  #html_document:
...
El objetivo del curso es desarrollar la capacidad de extraer información a partir de datos; resumir esta información, comunicar de manera efectiva, y usar para predecir y tomar decisiones. Así tambien, aprender a usar las tecnologías de visualización, análisis y manipulación de datos masivos más importantes en aplicaciones relevantes.

Aunque existen numerosas herramientas que facilitan la visualización, análisis y gestión de datos, **R sobresale por ser una alternativa adaptable, potente, sencilla de manejar, extensamente documentada, versátil y constantemente actualizada**. Además, es una alternativa totalmente gratuita. Es el instrumento preferido por los estadísticos y se utiliza extensamente en el ámbito tecnológico y empresarial.

Cabe mencionar que *"el foco del curso no está puesto en usar R como un lenguaje de programación. Más bien, está puesto en usar R como una herramienta para responder preguntas a través de datos, gentilmente incorporando elementos del pensamiento algorítimico cuando sea necesario"*.

Disclaimer: Este docuemnto no pretende ser un resumen completo de R y de todas las características que proporciona. No busca ser un libro de estadísticas ni una guía de buenas prácticas para transmitir información obtenida de datos. En realidad, **intenta condensar en un solo sitio los métodos de R más esenciales, y proporcionar numerosos ejemplos para simplificar el proceso de aprendizaje**.

Se recomenda a los interesados en ahondar sus conocimientos en R los siguienes libros: 
...
* Introducción a R
  + Primeros pasos en R
  + Usando R a través del entorno RStudio
  + R Scripts
  + R Notebooks
* Tablas de datos 
...
* Estadística descriptiva e inferencial 
  + Análisis de datos. Componentes básicos
  + Medidas descriptivas 
  + Probabilidad de ocurrencia 
  + Normalidad. Detección de la normalidad
  + Pruebas de Normalidad en RStudio


# Primeros pasos en R

R es un lenguaje de programación diseñado específicamente para realizar análisis estadístico. Una de sus principales características es que es **un software de código abierto**, lo que implica que se ofrece de forma gratuita y cualquiera que lo quiera tiene la posibilidad de examinar su código y sugerir mejoras. Por lo tanto, es que R cuenta con una amplia comunidad de usuarios donde se puede hallar apoyo y nuevas funcionalidades.

Se puede saber más de lo que actualmente es R, se puede leer [aquí](https://www.r-project.org/about.html).

## Usando R a través del entorno RStudio

El método más sencillo para manejar un lenguaje de programación es mediante un **Entorno Integrado de Desarrollo (IDE)**, un programa que facilita la escritura de código, su ejecución, la visualización de los resultados logrados y su depuración. *Para R, uno de los IDE más reconocidos es RStudio*.

Sigue las instrucciones siguientes para preparar tu ordenador para operar en R.

### Instalación de R y RStudio

1. Visita el sitio <https://cran.rstudio.com/>, donde hallará ediciones de R para diferentes sistemas operativos.
2. Visita <https://posit.co/download/rstudio-desktop/> y accede a la sección "Segunda etapa: Instalación de RStudio Desktop". Elige el instalador adecuado para tu sistema operativo, descárgalo e instálelo en su ordenador.

Asi tambien, para ambos pasos se tiene el siguiente [video](https://www.youtube.com/watch?v=OOkA6xkjsLc) para hacerlo paso a paso el proceso de descarga e instalación.

### Una opción en la nube: Posit Cloud

Otra forma más ágil de comenzar a utilizar RStudio es con [Posit Cloud](https://posit.cloud/), un servicio en línea que facilita la ejecución de RStudio directamente en un navegador, evitando la necesidad de hacer ninguna instalación en el ordenador. Considera que esta alternativa requiere de una conexión a internet para funcionar.

## El entorno RStudio

Al abrir RStudio, aparecerá una pantalla similar a la siguiente:

![Pantalla de la interfaz de RStudio](img/Pantalla Rstudio.png){width='900px'}

Esta pantalla presenta 4 apartados, cada apartado cuenta con varias pestañas, pero nos enfocaremos en las más relevantes:

1.	**Consola**: En esta parte puedes establecer una interacción directa con R, redactando instrucciones tras el símbolo `>`. Se puede hacer la prueba, escribiendo `2+2` en la consola y después pulsando la tecla `Enter`. R tiene la capacidad de actuar como una calculadora. No obstante, el método principal que sugerimos para interactuar con R es mediante archivos especiales (R Notebooks o R Scripts). **En la práctica solo se utiliza la consola para llevar a cabo ciertas tareas específicas, como la instalación de funciones en R**.
2.	**Navegador de archivos**: Aquí se tiene **la posibilidad de visualizar y rotar entre los archivos y carpetas presentes en el ordenador**. Si pulsas en los **tres puntos a la derecha (...) tienes la opción de elegir qué carpeta deseas mostrar en el navegador** de archivos. También está la opción de ayuda (help) y gráficos (plot).
3.	**Ambiente**: En esta parte se presentarán las variables y bases de datos (tablas, valores, objetos) que se utilizara en R.
4.	**Scripts o archivos en R**: Se puede disponer de diferentes archivos propios de RStudio, entre los más usados es R script, R notebooks, R markdown, etc.

## R Notebooks

La forma o medio para interactuar con R será mediante R Notebooks, este archivo nos permiten intercalar texto y comandos de R, además de visualizar los resultados de ejecutar estos comandos de forma cómoda.

Para crear un nuevo R Notebook, pulsa el menú desplegable en **el triángulo negro situado junto al ícono de nuevo archivo**, y posteriormente elige "R Notebook":

![Crear un nuevo R Notebook](img/crear-r-notebook.png){width='500px'}

A continuación, se crea un archivo R Notebook que se asemeja a un archivo de texto plano. Este documento ya contiene un cierto contenido de texto y código, que presenta un resumen (en inglés) de cómo manejar este tipo de archivo.

Si es tu primera creación de un archivo R Notebook, **RStudio podría solicitarte la instalación de ciertas librerías (esto se mostrará en un mensaje de color amarillo, encima de la pantalla)**. En tal situación, pulsa "Install" y aguarda a que RStudio ejecute la instalación de manera automática.

### Partes de un R Notebook

En la siguiente imagen se muestra la estructura típica de un archivo R Notebook.

![Partes de un R Notebook](img/estructura-r-notebook.png){width='600px'}

De manera general, las partes de un R Notebooks contienen los siguientes elementos:

* **Encabezado**: Este conjunto de líneas, encerrado entre `---`, contiene información general acerca del archivo (título, autor, fecha) junto con algunos parámetros que RStudio puede usar para procesarlo. Por ahora, lo único que te recomendamos hacer es poner un título a tu archivo cambiando el texto que está entre comillas al lado de `title:`.
* **Texto**: Este es texto común y corriente, que puedes usar para escribir algunos comentarios o conclusiones acera del análsis que estés realizando en este archivo. Poner texto en un archivo R Notebook es una muy buena de documentar lo que estés haciendo (tanto para tu propia referencia como para otras personas con las que compartas este archivo), incluso el mismo archivo R Notebook puede transformarse en un informe completamente autocontenido. 
* **Chunk de código**: Este subconjunto de líneas está encerrado entre ` ```{r}` y ` ``` ` y en la jerga de RStudio recibe el nombre de **“chunk” (o trozo) de código**. Aquí es donde podrás ingresar código en R, ejecutarlo y ver los resultados obtenidos. Puedes crear **cuantos chunks quieras a lo largo del documento**, pero te recomendamos no crearlos manualmente sino que
  + usando el menú `“Code” > “Insert chunk”`,
  + haciendo click en **el botón verde en el parte superior derecha** del editor con un **signo + y una letra “C”**, o
  + usando el shortcut de teclado  `Ctrl + Alt + I` (Windows).

Para ejecutar el código contenido en un chunk puedes hacer click en el símbolo de reproducción verde arriba a la derecha de cada chunk (triángulo verde). También puedes usar el shortcut de teclado `Control + Shift + Enter` (Windows). Haz la prueba con el chunk que contiene el código plot(cars) y verás que R mostrará un gráfico de dispersión (realizado a partir de una base de datos precargada en R llamada cars) justo abajo.

![Ejecución de un chunk](img/ejecutando-un-chunk.png){width='600px'}
